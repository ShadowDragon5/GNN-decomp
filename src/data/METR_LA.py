...
class METR_LA(Dataset):
    ...
    def get_index_dataset(
        ...
        data = np.load(os.path.join(self.raw_data_dir, "node_values.npy")).transpose(
            (1, 2, 0)
        )
        ...
        train_dataset = self.IndexDataset(
            x_train, data, lags, gpu=not (allGPU == -1), lazy=dask_batching
        )
        val_dataset = self.IndexDataset(
            x_val, data, lags, gpu=not (allGPU == -1), lazy=dask_batching
        )
        test_dataset = self.IndexDataset(
            x_test, data, lags, gpu=not (allGPU == -1), lazy=dask_batching
        )

        if ddp_rank != -1:
            train_sampler = DistributedSampler(
                train_dataset, num_replicas=world_size, rank=ddp_rank, shuffle=shuffle
            )
            ...
            val_sampler = DistributedSampler(
                val_dataset, num_replicas=world_size, rank=ddp_rank, shuffle=shuffle
            )
            ...
            test_sampler = DistributedSampler(
                test_dataset, num_replicas=world_size, rank=ddp_rank, shuffle=shuffle
            )
