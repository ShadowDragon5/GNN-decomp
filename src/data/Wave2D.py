"""
Taken and adapted from "Learning to Solve PDE-constrained Inverse Problems with Graph Networks"
Repository: https://github.com/computational-imaging/GraphPDE.git
...
class wave_data_2D_irrgular(Dataset):
    ...
    def get_raw(self, idx, num_timesteps_pertraj, trajectory_dataset):
        ...
        input_node_features = {}
        input_edge_features = {}
        input_edge_features["dist"] = torch.tensor(
            np.sqrt(
                np.sum(
                    (coords[edge_lists[1, :]] - coords[edge_lists[0, :]]) ** 2, axis=-1
                )
            )
        ).unsqueeze(-1)
        input_edge_features["direction"] = torch.tensor(
            (coords[edge_lists[0, :]] - coords[edge_lists[1, :]])
        )

        if "density" in self.node_features:
            input_node_features["density"] = (
                torch.tensor(traj["density_mesh"])
                .view(-1, num_nodes)
                .squeeze()
                .unsqueeze(-1)
            )
            input_node_features["density_eval"] = (
                torch.tensor(traj["density_mesh"])
                .view(-1, num_nodes)
                .squeeze()
                .unsqueeze(-1)
        ...
        unroll_v_gt = (
            U_solution[i + 1 : i + 1 + 1, :] - U_solution[i : i + 1, :]
        ).permute(1, 0)
        ...
        input_node_features["v_eval"] = (
            input_node_features["u_eval"] - input_node_features["u_h_eval"]
        )
        input_node_features["v_gt"] = (
            U_solution[i + 1 : i + 1 + 1, :] - U_solution[i : i + 1, :]
        ).permute(1, 0)
        input_node_features["v_gt_eval"] = (
            U_eval[i + 1 : i + 1 + 1, :] - U_eval[i : i + 1, :]
        ).permute(1, 0)
        input_node_features["a_gt"] = (
            input_node_features["v_gt"] - input_node_features["v"]
        )
        input_node_features["a_gt_eval"] = (
            input_node_features["v_gt_eval"] - input_node_features["v_eval"]
        )

        x = input_node_features[self.node_features[0]].float()
        for feature in self.node_features[1:]:
            x = torch.cat([x, input_node_features[feature]], dim=-1).float()

        x_eval = input_node_features[self.node_features[0] + "_eval"].float()
        for feature in self.node_features[1:]:
            x_eval = torch.cat(
                [x_eval, input_node_features[feature + "_eval"]], dim=-1
            ).float()

        edge_attr = input_edge_features[self.edge_features[0]].float()
        for feature in self.edge_features[1:]:
            edge_attr = torch.cat(
                [edge_attr, input_edge_features[feature]], dim=-1
            ).float()

        data = Data(
            x=x,
            edge_index=edge_lists,
            edge_attr=edge_attr,
            current_u=input_node_features["u"],
            h=input_node_features["u_h"],
            gt=input_node_features["u_gt_eval"],
            v_gt=input_node_features["v_gt_eval"],
            x_eval=x_eval,
            cell=edges,
            num_nodes=num_nodes,
...
#
#     datalist = []
#     for _, traj in enumerate(trajectory_dataset):
#         if prefix == "high":
#             U_solution = torch.tensor(traj["solution"][0:endtime:step_size])
#         else:
#             U_solution = torch.tensor(
#                 traj["solution{}".format(prefix)][0:endtime:step_size]
#             )
#
...
#                 np.array(traj["is_boundary{}".format(prefix)])
#             ).reshape(-1, 1)
#
...
#                 np.sqrt(
#                     np.sum(
#                         (coords[edge_lists[1, :]] - coords[edge_lists[0, :]]) ** 2,
#                         axis=-1,
#                     )
#                 )
#             ).unsqueeze(-1)
...
#             # unroll_u_gt = (U_solution[i + 1 : i + 1 + 1, :]).permute(1, 0)
#             input_node_features["u_gt"] = U_solution[i + 1 : i + 1 + 1, :].permute(1, 0)
#             input_node_features["u_gt_eval"] = U_eval[i + 1 : i + 1 + 1, :].permute(
#                 1, 0
#             )
#             input_node_features["v"] = (
#                 input_node_features["u"] - input_node_features["u_h"]
#             )
#             input_node_features["v_eval"] = (
#                 input_node_features["u_eval"] - input_node_features["u_h_eval"]
#             )
...
#             for feature in node_features[1:]:
#                 x_eval = torch.cat(
#                     [x_eval, input_node_features[feature + "_eval"].float()], dim=-1
#                 )
#
#             edge_attr = input_edge_features[edge_features[0]].float()
#             for feature in edge_features[1:]:
#                 edge_attr = torch.cat(
#                     [edge_attr, input_edge_features[feature].float()], dim=-1
#                 )
#
#             traj["fishcount"] = None
#             data = Data(
...
def wave_data_update(
    node_features, output_graph, old_graph, output_type="v", train=True, keep_grad=False
):
    """
    Parameters
    ----------
    ...
    if output_type == "x":
        ...
        node_feature_list["u_eval"] = (
            output_graph.eval[:, -1] + old_graph.x_eval[:, 0]
        )  # v*1+x
        ...
        node_feature_list["u"] = (
            output_graph.x[:, -1] + old_graph.x[:, 1] + old_graph.x[:, 0]
        )  # a*1+v+x
        node_feature_list["u_eval"] = (
            output_graph.eval[:, -1] + old_graph.x_eval[:, 1] + old_graph.x_eval[:, 0]
        )  # a*1+v+x
        node_feature_list["v"] = output_graph.x[:, -1] + old_graph.x[:, 1]  # a+v
        node_feature_list["v_eval"] = (
            output_graph.eval[:, -1] + old_graph.x[:, 1]
        )  # a+v
...
#                 np.load(
#                     "./data/prior/density_10000_{}_5_10.npy".format(
#                         int(sidelength / 2)
#                     ),
#                     allow_pickle=True,
#                 )
...
#             self.density = torch.tensor(
#                 np.load(
#                     "./data/prior/initial_states_10000_{}_6_10.npy".format(
#                         int(sidelength / 2)
#                     ),
#                     allow_pickle=True,
#                 )
